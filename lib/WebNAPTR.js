   ...
   generate : function(service_enum) {
      ...
      switch(service_enum) {
         ...
         case this.Services.IFAX:
            service = '"E2U+ifax:mailto"';
            regServ = 'mailto:';
            break;
         default:
            this.error = "Wrong service id has been entered";
            return false;
            break;
      }

      // form record
      record += service + ' "!' + (this.regexp ? this.regexp : '^.*$') + '!' 
            //  ^service           ^regexp                                     
            + regServ + ':' + this.uri + '!' + (this.ignorecase? 'i' : '') + '"';
            //^service used in regexp          ^ignorecase
