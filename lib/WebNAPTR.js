...
var Webnaptr = {
   ...
   */
   ...
   /**
    * Set first part of regexp - which strings should be affected
    * @param regexp regexp containing only first part of regular expression
    */
   ...
   generate : function(service_enum) {
      ...
      switch(service_enum) {
         case this.Services.H323:
            record += '"E2U+H323" "!' + (this.regexp ? this.regexp : '^.*$') +'!h323:' + this.uri + '!"';
            //          ^service        ^regexp
            break;
         case this.Services.SIP:
            record += '"E2U+sip" "!' + (this.regexp ? this.regexp : '^.*$') +'!sip:' + this.uri + '!"';
            break;
         case this.Services.EMAIL:
            record += '"E2U+email" "!' + (this.regexp ? this.regexp : '^.*$') +'!mailto:' + this.uri + '!"';
            break;
         case this.Services.IFAX:
            record += '"E2U+ifax:mailto" "!' + (this.regexp ? this.regexp : '^.*$') +'!mailto:' + this.uri + '!"';
            break;
         default:
            break;
      }
